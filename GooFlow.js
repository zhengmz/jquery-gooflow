...
//初始化区域图的对象
	...
	if(property.haveExpend===undefined){
		property.haveExpend=true;
	}
	if(property.haveExpend){
		this.initExpendFunc();
	}
	...
	addNode:function(id,json){
		...
		if(GooFlow.prototype.color.node){
			...
			if(mark&&GooFlow.prototype.color.mark){
				this.$nodeDom[id].css({"border-color":GooFlow.prototype.color.mark,"background-color":GooFlow.prototype.color.mark});
				this.$nodeDom[id].find("td:eq(1)").css("color",GooFlow.prototype.color.markFont);
			}
		}
		if(json.type.indexOf(" mix")>-1){
...
///////////以下为有关画线的方法
	...
	setLineType:function(id,newType,M){
		//modify at 2022-6-24
		if (id === undefined) {
			console.log('Please re-choice line ...');
			return false;
		}
		//modify end
